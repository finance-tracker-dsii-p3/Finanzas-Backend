...
def custom_exception_handler(exc, context):
    ...
    if response is not None:
        ...
        if response.status_code == 400:
            custom_response_data['suggestion'] = 'Verifica que todos los campos requeridos estén presentes y sean válidos'
        elif response.status_code == 401:
            custom_response_data['suggestion'] = 'Incluye el token de autenticación en el header Authorization'
        elif response.status_code == 403:
            custom_response_data['suggestion'] = 'Verifica que tengas permisos para realizar esta acción'
        elif response.status_code == 404:
            custom_response_data['suggestion'] = 'Verifica que el ID del recurso sea correcto y te pertenezca'
