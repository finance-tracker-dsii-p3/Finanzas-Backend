...
def test_serializer():
    ...
    try:
        ...
        print(
            f"   - categories_count: {expenses_data.get('categories_count')} (tipo: {type(expenses_data.get('categories_count'))})"
        )
        ...
        try:
            ...
            for key, value in serializer.data.items():
                if isinstance(value, (list, dict)):
                    print(
                        f"   {key}: {type(value)} (elementos: {len(value) if isinstance(value, list) else 'dict'})"
                    )
                else:
