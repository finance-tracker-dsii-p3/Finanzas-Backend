...
class TransactionDetailSerializer(serializers.ModelSerializer):
    ...
    origin_account_currency = serializers.CharField(source='origin_account.currency', read_only=True)
    destination_account_name = serializers.CharField(source='destination_account.name', read_only=True, allow_null=True)
    goal_name = serializers.CharField(source='goal.name', read_only=True, allow_null=True)  # HU-11
    ...
    class Meta:
        ...
        fields = [
            ...
            "goal_name",  # HU-11: Nombre de la meta
...
class TransactionSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if origin_account:
            final_total_pesos = Decimal(str(final_total)) / Decimal('100') if final_total else Decimal('0')
...
class TransactionUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        goal = data.get('goal', self.instance.goal if hasattr(self, 'instance') and self.instance else None)
        if goal:
            if transaction_type != 4:
                raise serializers.ValidationError({
                    'goal': 'Solo se pueden asignar metas a transacciones tipo Saving (type=4).'
        ...
        if origin_account:
            account_currency = origin_account.currency
            transaction_currency = data.get('transaction_currency', self.instance.transaction_currency if hasattr(self, 'instance') and self.instance else None)
