    ...
    def check_and_create_alerts():
        ...
        for soat in soats:
            ...
            recent_alert = SOATAlert.objects.filter(
                soat=soat, created_at__gte=timezone.now() - timezone.timedelta(hours=24)
            ).exists()
            ...
            if soat.status == "atrasado":
                ...
                soat.status == "pendiente_pago"
                and days is not None
                and days <= soat.alert_days_before
            ):
                ...
                soat.status == "por_vencer" and days is not None and days <= soat.alert_days_before
            ):
            ...
            if alert_type and message:
                alert = SOATAlert.objects.create(
                    soat=soat, user=user, alert_type=alert_type, message=message
                )
                ...
                try:
                    ...
                    if alert_type in ["atrasada", "vencida"]:
                        ...
                        if days is not None and days <= 7:
                            NotificationEngine.create_soat_reminder(
                                user=user, soat=soat, alert_type="due_soon", days=days
                            )
                        else:
                            NotificationEngine.create_soat_reminder(
                                user=user, soat=soat, alert_type="upcoming", days=days
                            )
                except Exception:
                    pass
