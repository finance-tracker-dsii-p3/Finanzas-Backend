...
class CustomReminderAdmin(admin.ModelAdmin):
    ...
    list_display = ['id', 'user', 'title', 'reminder_date', 'reminder_time', 'is_sent', 'is_read', 'created_at']
    ...
    fieldsets = (
        ...
        ('Programación', {
            'fields': ('reminder_date', 'reminder_time')
        }),
        ('Estado', {
            'fields': ('is_sent', 'sent_at', 'notification', 'is_read', 'read_at')
        }),
        ('Auditoría', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
