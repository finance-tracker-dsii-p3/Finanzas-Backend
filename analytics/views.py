...
def analytics_dashboard(request):
    ...
    try:
        ...
        if mode not in ["base", "total"]:
            return Response(
                {"error": 'Modo inválido. Debe ser "base" o "total"', "code": "INVALID_MODE"},
        ...
        if total_transactions == 0:
            ...
            )  # 200 porque no es error del sistema
...
def period_indicators(request):
    ...
    try:
        ...
        return Response(
            {"success": True, "data": serializer.data, "message": message},
            status=status.HTTP_200_OK,
        )
...
def expenses_by_category(request):
    ...
    try:
        ...
        period_expenses = Transaction.objects.filter(
            user=user, type=2, date__gte=start_date, date__lte=end_date
        ).count()
    ...
    try:
        ...
        if category_id != "uncategorized":
            try:
                ...
                return Response(
                    {"error": "ID de categoría inválido"}, status=status.HTTP_400_BAD_REQUEST
                )
        ...
        if "error" in transactions_data:
            return Response(
                {"success": False, "error": transactions_data["error"]},
...
def available_periods(request):
    ...
    try:
        ...
        date_range = Transaction.objects.filter(user=request.user).aggregate(
        ...
        periods = [
            {"key": "current_month", "name": "Mes actual", "description": today.strftime("%B %Y")},
            {"key": "last_month", "name": "Mes anterior", "description": "Mes completo anterior"},
            {"key": "current_year", "name": "Año actual", "description": str(today.year)},
            {"key": "last_7_days", "name": "Últimos 7 días", "description": "Semana reciente"},
            {"key": "last_30_days", "name": "Últimos 30 días", "description": "Mes reciente"},
        ]
    ...
    try:
            ...
            return Response(
                {
                    ...
                    "details": {
                        "provided": {"period1": period1_str, "period2": period2_str},
                        "examples": [
                            "?period1=2025-09&period2=2025-10&mode=total",
                            "?period1=last_month&period2=current_month&mode=base",
        ...
        if not comparison_data["comparison_summary"]["can_compare"]:
            ...
            if not period1_has_data and not period2_has_data:
                error_msg = "Ninguno de los períodos tiene transacciones"
