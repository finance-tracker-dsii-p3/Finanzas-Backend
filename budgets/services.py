...
class BudgetService:
    ...
    def get_budget_stats(user):
        ...
        # Totales
        total_allocated = active_budgets.aggregate(
            total=Sum('amount')
    ...
    @staticmethod
    def create_budget(user, category, amount, calculation_mode=Budget.BASE,
                     period=Budget.MONTHLY, start_date=None, is_active=True,
    ...
    def update_budget(budget, **kwargs):
        ...
        logger.info(
            f'Presupuesto actualizado: {budget.id} - Usuario: {budget.user.id}'
        )
    ...
    def toggle_active(budget):
        ...
        logger.info(
            f'Presupuesto {"activado" if budget.is_active else "desactivado"}: '
            f'{budget.id}'
        )
    ...
    def get_categories_without_budget(user, period=Budget.MONTHLY):
        ...
        budgeted_category_ids = Budget.objects.filter(
            user=user,
            period=period
