...
class CategoryModelTest(TestCase):
    ...
    def test_same_name_different_type_allowed(self):
        ...
        Category.objects.create(
            user=self.user, name="Regalos", type=Category.INCOME, color="#059669"
        )
    ...
    def test_ordering(self):
        ...
        cat1 = Category.objects.create(
            user=self.user, name="Z Último", type=Category.EXPENSE, color="#DC2626", order=3
        )

        cat2 = Category.objects.create(
            user=self.user, name="A Primero", type=Category.EXPENSE, color="#059669", order=1
        )

        cat3 = Category.objects.create(
            user=self.user, name="M Medio", type=Category.EXPENSE, color="#2563EB", order=2
        )
