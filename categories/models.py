...
class Category(models.Model):
    ...
    name = models.CharField(
        max_length=100, help_text="Nombre de la categoría (ej: Comida, Salario)"
    )

    type = models.CharField(
        max_length=10, choices=TYPE_CHOICES, help_text="Tipo de categoría: ingreso o gasto"
    )
    ...
    is_active = models.BooleanField(
        default=True, help_text="Indica si la categoría está activa y disponible para usar"
    )

    is_default = models.BooleanField(
        default=False, help_text="Indica si es una categoría del sistema (no editable por usuario)"
    )
    ...
    def clean(self):
        ...
        existing = Category.objects.filter(user=self.user, name__iexact=self.name, type=self.type)
