...
class CategoryViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        logger.info(f"Usuario {request.user.id} listó categorías: {count} encontradas")
        ...
        if serializer.is_valid():
            try:
                ...
                result = CategoryService.delete_category(
                    category=category, target_category_id=target_category_id
                )
    ...
    def toggle_active(self, request, pk=None):
        ...
        try:
            ...
            logger.info(f"Usuario {request.user.id} {action_text} categoría: {category.name}")
    ...
    def stats(self, request):
        ...
        try:
            ...
            logger.info(f"Generadas estadísticas de categorías para usuario {request.user.id}")
        ...
        if serializer.is_valid():
            try:
                ...
                updated_count = CategoryService.bulk_update_order(
                    user=request.user, order_data=order_data
                )
    ...
    def income(self, request):
        ...
        categories = CategoryService.get_user_categories(
            user=request.user, include_inactive=False, category_type=Category.INCOME
        )

        serializer = CategoryListSerializer(categories, many=True)

        count = categories.count()
        logger.info(f"Usuario {request.user.id} listó categorías de ingresos: {count} encontradas")
    ...
    def expense(self, request):
        ...
        """
        categories = CategoryService.get_user_categories(
            user=request.user, include_inactive=False, category_type=Category.EXPENSE
        )

        serializer = CategoryListSerializer(categories, many=True)

        count = categories.count()
        logger.info(f"Usuario {request.user.id} listó categorías de gastos: {count} encontradas")
