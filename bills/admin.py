...
class BillAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Información básica", {
            "fields": ("user", "provider", "amount", "due_date")
        }),
        ("Cuentas y categorías", {
            "fields": ("suggested_account", "category")
        }),
        ("Estado y pago", {
            "fields": ("status", "payment_transaction")
        }),
        ("Configuración", {
            "fields": ("reminder_days_before", "is_recurring", "description")
        }),
        ("Fechas", {
...
class BillReminderAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Recordatorio", {
            "fields": ("user", "bill", "reminder_type", "message")
        }),
        ("Estado", {
