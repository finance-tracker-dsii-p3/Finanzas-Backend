        ...
        try:
            ...
            for row_num, user in enumerate(queryset.order_by("username"), 2):
                ...
                worksheet.cell(
                    row=row_num,
                    column=11,
                    value=user.date_joined.strftime("%Y-%m-%d %H:%M") if user.date_joined else "",
                )
                worksheet.cell(
                    row=row_num,
                    column=12,
                    value=user.last_login.strftime("%Y-%m-%d %H:%M") if user.last_login else "",
                )
    ...
    def export_notifications_data(self):
        ...
        try:
            ...
            for row_num, notification in enumerate(queryset.order_by("-created_at"), 2):
                ...
                worksheet.cell(
                    row=row_num, column=3, value=notification.get_notification_type_display()
                )
                ...
                worksheet.cell(
                    row=row_num, column=7, value=notification.created_at.strftime("%Y-%m-%d %H:%M")
                )
                worksheet.cell(
                    row=row_num,
                    column=8,
