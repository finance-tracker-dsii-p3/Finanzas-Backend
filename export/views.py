...
def download_export(request, export_id):
    ...
    try:
        export_job = get_object_or_404(
            ExportJob, id=export_id, requested_by=request.user, status=ExportJob.COMPLETED
        )
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="{export_job.file.name.split("/")[-1]}"'
        )
        return response
...
def create_basic_export(request):
    ...
    try:
        ...
        title = request.data.get(
            "title", f"Exportación {timezone.now().strftime('%Y-%m-%d %H:%M')}"
        )
        format_type = request.data.get("format", "excel")

        # Crear job de exportación
        export_job = ExportJob.objects.create(
            title=title, export_type=export_type, format=format_type, requested_by=request.user
        )
