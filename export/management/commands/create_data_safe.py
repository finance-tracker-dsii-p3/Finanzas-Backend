...
class Command(BaseCommand):
    help = "Crear datos simulados usando usuarios existentes SIN borrar nada"

    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS("Creando datos simulados usando usuarios existentes...")
        )
        ...
        try:
            ...
            monitors = users.filter(role=User.MONITOR)

            if not monitors.exists():
                self.stdout.write(
                    self.style.ERROR(
                        "No hay monitores disponibles. Se necesitan monitores para crear datos."
                    )
                )
                return

            if not admins.exists():
                self.stdout.write(self.style.ERROR("No hay administradores disponibles."))
                return

            self.stdout.write(
                f"Usando {admins.count()} administradores y {monitors.count()} monitores existentes"
            )

            # Crear datos usando usuarios existentes
            ...
            self.stdout.write(f"Usuarios utilizados: {users.count()} (sin crear nuevos)")
            ...
            self.stdout.write("PERIODO DE DATOS: Agosto y Septiembre 2025")
    ...
    def create_rooms(self):
        ...
        for room_data in rooms_data:
            room, created = Room.objects.get_or_create(code=room_data["code"], defaults=room_data)
            if created:
            ...
            if current_date.weekday() < 5:
                ...
                for shift in selected_shifts:
                    ...
                    start_time = datetime.combine(
                        current_date, datetime.strptime(shift["start"], "%H:%M").time()
                    )
                    end_time = datetime.combine(
                        current_date, datetime.strptime(shift["end"], "%H:%M").time()
                    )
    ...
    def create_equipment_reports(self, equipment, monitors):
        ...
        for _i in range(num_reports):
            ...
            report_time = datetime.combine(
                report_date,
                datetime.strptime(
                    f"{random.randint(6, 20)}:{random.randint(0, 59):02d}", "%H:%M"
                ).time(),
            )
    ...
    def create_notifications(self, monitors):
        ...
        for i in range(num_notifications):
            ...
            notification_time = datetime.combine(
                notification_date,
                datetime.strptime(
                    f"{random.randint(8, 18)}:{random.randint(0, 59):02d}", "%H:%M"
                ).time(),
            )
