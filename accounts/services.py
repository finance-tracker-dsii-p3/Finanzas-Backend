...
class AccountService:
    ...
    def get_accounts_summary(user):
        ...
        for account in accounts:
            ...
            if account.account_type == Account.ASSET:
                ...
                balances_by_currency[account.currency]["liabilities"] += abs(
            ...
            if category_name not in accounts_by_category:
                accounts_by_category[category_name] = {"count": 0, "balance": Decimal("0.00")}
    ...
    def get_accounts_by_currency(user, currency):
        ...
        """
        return Account.objects.filter(user=user, currency=currency, is_active=True).order_by("name")
    ...
    def get_credit_card_details(account):
        ...
        used_credit = (
            abs(account.current_balance) if account.current_balance < 0 else Decimal("0.00")
        )
    ...
    def get_credit_cards_summary(user):
        ...
        credit_cards = Account.objects.filter(
            user=user, category=Account.CREDIT_CARD, is_active=True
        )

        total_used_credit = sum(abs(card.current_balance) for card in credit_cards)

        # Calcular límite total de crédito
        total_credit_limit = sum(
            card.credit_limit for card in credit_cards if card.credit_limit is not None
        ) or Decimal("0.00")

        # Calcular crédito disponible
        available_credit = (
            total_credit_limit - total_used_credit if total_credit_limit > 0 else Decimal("0.00")
        )
