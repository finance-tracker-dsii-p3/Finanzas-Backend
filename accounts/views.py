...
class AccountViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        try:
            ...
            logger.warning(
                f'Error al crear cuenta para usuario {self.request.user.id}: {str(e)}'
            )
            raise e
    ...
    def perform_update(self, serializer):
        ...
        try:
            ...
            logger.info(
                f'Usuario {self.request.user.id} actualizó cuenta: {account.name}'
            )
            
        except Exception as e:
            logger.warning(
                f'Error al actualizar cuenta {self.get_object().id}: {str(e)}'
            )
            raise e
    ...
    def perform_destroy(self, instance):
        ...
        try:
            ...
            logger.info(
                f'Usuario {self.request.user.id} eliminó cuenta: {instance.name}'
            )
            
        except ValueError as e:
            logger.warning(
                f'Error al eliminar cuenta {instance.id}: {str(e)}'
            )
            raise e
    ...
    def by_currency(self, request):
        ...
        if currency not in dict(Account.CURRENCY_CHOICES):
            return Response(
                {'error': f'Moneda {currency} no válida'}, 
                status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def credit_cards_summary(self, request):
        ...
        try:
            ...
            logger.info(
                f'Generado resumen de tarjetas para usuario {request.user.id}'
            )
    ...
    def update_balance(self, request, pk=None):
        ...
        if serializer.is_valid():
            try:
                ...
                logger.warning(
                    f'Error al actualizar saldo de cuenta {account.id}: {str(e)}'
                )
                return Response(
                    {'error': str(e)}, 
                    status=status.HTTP_400_BAD_REQUEST
    ...
    def validate_deletion(self, request, pk=None):
        ...
        try:
            ...
            logger.error(
                f'Error al validar eliminación de cuenta {account.id}: {str(e)}'
            )
            return Response(
    ...
    def toggle_active(self, request, pk=None):
        ...
        try:
            ...
            logger.info(
                f'Usuario {request.user.id} {action_text} cuenta: {account.name}'
            )
            ...
            logger.error(
                f'Error al cambiar estado de cuenta {account.id}: {str(e)}'
            )
            return Response(
    ...
    def list(self, request, *args, **kwargs):
        ...
        logger.info(
            f'Usuario {request.user.id} listó cuentas: {queryset.count()} encontradas'
        )
