...
def update_goal_on_saving_transaction(sender, instance, **kwargs):
    ...
    try:
        ...
        logger.error(f"Error actualizando meta {instance.goal.id} con transacción {instance.id}: {str(e)}")
...
def remove_transaction_from_goal(sender, instance, **kwargs):
    ...
    try:
        ...
        logger.error(f"Error removiendo transacción {instance.id} de meta {instance.goal.id}: {str(e)}")
