...
class TransactionsAPIWorkflowTests(TestCase):
    ...
    def test_create_transaction_endpoint_exists(self):
        ...
        response = self.client.post(
            "/api/transactions/", data=json.dumps(transaction_data), **self.auth_headers
        )
    ...
    def test_transaction_model_has_hu12_fields(self):
        ...
        assert hasattr(transaction, "applied_rule")
    ...
    def test_transaction_with_category_creation(self):
        ...
        assert transaction.description == "Uber centro"
        assert transaction.category == self.test_category
        assert transaction.applied_rule is None  # No rule applied yet
    ...
    def test_transaction_filters_exist(self):
        ...
        response = self.client.get(
            f"/api/transactions/?category={self.test_category.id}", **self.auth_headers
        )
        assert response.status_code != 404

        # Test filtro por descripción (búsqueda)
        response = self.client.get("/api/transactions/?search=test", **self.auth_headers)
        assert response.status_code != 404
...
class TransactionsRulesIntegrationWorkflowTests(TestCase):
    ...
    def setUp(self):
        ...
        self.category = Category.objects.create(user=self.user, name="Transporte", type="expense")
    ...
    def test_transaction_can_reference_rule(self):
        ...
        assert transaction.applied_rule == self.rule
    ...
    def test_rule_can_find_applied_transactions(self):
        ...
        assert applied_transactions.count() == 1
        assert applied_transactions.first().description == "Taxi aeropuerto"
    ...
    def test_transaction_model_save_method_exists(self):
        ...
        assert hasattr(transaction, "save")
