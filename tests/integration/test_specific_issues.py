...
def test_specific_issues():
    ...
    try:
        response = requests.patch(
            f"{base_url}/api/rooms/entry/{entry_id}/exit/",
            json={"notes": "Prueba de salida específica"},
            headers=headers,
        )
        print(f"   Status: {response.status_code}")
        print(f"   Response: {response.text}")
    ...
    try:
        response = requests.patch(
            f"{base_url}/api/rooms/my-active-entry/exit/",
            json={"notes": "Prueba de salida alternativa"},
            headers=headers,
        )
        print(f"   Status: {response.status_code}")
        print(f"   Response: {response.text}")
