...
class RulesAPIWorkflowTests(TestCase):
    ...
    def setUp(self):
        ...
        self.test_category = Category.objects.create(
            user=self.user, name="Transporte", type="expense", color="#EA580C", icon="fa-car"
        )
    ...
    def test_rules_endpoint_exists(self):
        ...
        assert response.status_code in [200, 401]  # 401 si hay problema de auth
    ...
    def test_create_rule_endpoint_exists(self):
        ...
        response = self.client.post("/api/rules/", data=json.dumps(rule_data), **self.auth_headers)
    ...
    def test_rule_model_creation(self):
        ...
        assert hasattr(rule, "matches_transaction")
    ...
    def test_rule_preview_endpoint(self):
        ...
        preview_data = {"description": "Test Uber ride"}

        response = self.client.post(
            "/api/rules/preview/", data=json.dumps(preview_data), **self.auth_headers
        )
...
class RulesModelWorkflowTests(TestCase):
    ...
    def setUp(self):
        ...
        self.category = Category.objects.create(
            user=self.user, name="Test Category", type="expense"
        )

    def test_automatic_rule_str_method(self):
        """Verificar que el método __str__ del modelo funciona"""
        rule = AutomaticRule(
            user=self.user, name="Test Rule Name", criteria_type=AutomaticRule.DESCRIPTION_CONTAINS
        )

        assert str(rule) == "Test Rule Name - Descripción contiene texto"

    def test_rule_choices_are_valid(self):
        """Verificar que las opciones del modelo están definidas"""
        assert hasattr(AutomaticRule, "DESCRIPTION_CONTAINS")
        assert hasattr(AutomaticRule, "TRANSACTION_TYPE")
        assert hasattr(AutomaticRule, "ASSIGN_CATEGORY")
        assert hasattr(AutomaticRule, "ASSIGN_TAG")
    ...
    def test_rule_validation_methods_exist(self):
        ...
        assert hasattr(rule, "matches_transaction")
        assert hasattr(rule, "apply_to_transaction")
        assert hasattr(rule, "clean")
