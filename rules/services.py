...
class RuleEngineService:
            ...
            for rule in rules:
                if rule.matches_transaction(transaction_obj):
                    ...
                    if rule_result['applied']:
                        ...
                        logger.info(f"Regla '{rule.name}' aplicada a transacción {transaction_obj.id}")
                        break
    ...
    @staticmethod
    def get_matching_rules(user, description: str = None, transaction_type: int = None) -> List[AutomaticRule]:
        """
    ...
    @staticmethod
    def preview_rule_application(user, description: str = None, transaction_type: int = None) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            'rule': {
                ...
                'target_category': first_rule.target_category.name if first_rule.target_category else None,
...
class AutomaticRuleService:
    ...
    def create_rule(user, validated_data: Dict[str, Any]) -> AutomaticRule:
        ...
        if 'order' not in validated_data or validated_data['order'] == 0:
            ...
            max_order = AutomaticRule.objects.filter(user=user).aggregate(
                models.Max('order')
            )['order__max'] or 0
    ...
    def reorder_rules(user, rule_orders: List[Dict[str, int]]) -> List[AutomaticRule]:
        ...
        with transaction.atomic():
            for item in rule_orders:
                try:
                    rule = AutomaticRule.objects.get(
                        id=item['id'], 
                        user=user
                    )
    ...
    def get_rule_statistics(user) -> Dict[str, Any]:
        ...
        recent_applications = Transaction.objects.filter(
            user=user,
            applied_rule__isnull=False
        ).select_related('applied_rule').order_by('-created_at')[:5]
